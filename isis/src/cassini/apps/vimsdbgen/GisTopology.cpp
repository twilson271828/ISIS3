...
GisTopology *GisTopology::getInstance() { 
  if (!m_gisfactory) {
...
GEOSGeometry *GisTopology::getGeomFromWKB(const std::string &wkb) {
...
GEOSGeometry *GisTopology::getGeomFromWKT(const std::string &wkt) {
...
const GEOSPreparedGeometry *GisTopology::getPreparedGeometry(const GEOSGeometry *geom) const {
...
std::string GisTopology::getWKT(const GEOSGeometry *geom, 
...
std::string GisTopology::getWKB(const GEOSGeometry *geom, 
                                const GisTopology::Disposition &disp) {
  size_t length;
  unsigned char *wkt_h = GEOSWKBWriter_writeHEX(getWKBWriter(), geom, &length);
  std::string thegeom(reinterpret_cast<const char *> (wkt_h), length);
...
GEOSWKTReader *GisTopology::getWKTReader() {
  if (!m_WKTreader) {
...
GEOSWKTWriter *GisTopology::getWKTWriter() {
  if (!m_WKTwriter) {
...
GEOSWKBReader *GisTopology::getWKBReader() {
  if (!m_WKBreader) {
...
GEOSWKBWriter *GisTopology::getWKBWriter() {
  if (!m_WKBwriter) {
